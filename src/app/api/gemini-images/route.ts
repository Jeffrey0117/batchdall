  ...
  try {
    ...
    if (!apiKey || apiKey.trim() === '') {
      ...
      const mockImages = Array.from({ length: n }, (_, i) => {
        const bgHue = Math.abs(
          [...(prompt || "mock")]?.reduce((acc: number, c: string) => acc + c.charCodeAt(0), seed || 7)
        ) % 360;
    ...
    console.log('OpenAI API Key provided, generating real images with DALL-E 3');
    ...
    try {
      ...
      console.log(`DALL-E 3 generated ${realImages.length} images successfully`);
      ...
      return Response.json({ 
        images: mockImages,
        error: `DALL-E 3 API 調用失敗: ${openaiError instanceof Error ? openaiError.message : '未知錯誤'}`,
        note: "已回退到模擬模式"
      });
    }
